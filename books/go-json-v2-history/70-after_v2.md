...
`format` タグは、フィールドごとに表現の形式を指定する仕組みでした。`time.Time` に `format:RFC1123` を付けて書式を選ぶ、`[]byte` に `format:base64url` を付けて符号化方式を選ぶ、そして `time.Duration` に `format:iso8601` や `format:units` を付ける。`time.Duration` の決着、つまり「既定を持たず、書き手に形式を選ばせる」は、この仕組みを前提にしていました。
...
>
> （Go 1.28 で何らかの形で型付き構造体タグが入る見込みであることから、json/v2 ワーキンググループは `format` タグオプションのサポートを削除することを決めた。これは型付き構造体タグとして表現するほうが自然だからである）
...
>
> （この提案を起動させ、優先度を上げるために、@rsc、@griesemer、@adonovan、@neild と会合を始めた。理由の一部は、この提案の見込みと時期が `json/v2` の決定に影響するからである）

その10日後に `#79071` が立ち、`format` タグが取り下げられます。言語側の未確定な提案が、標準ライブラリの提案から機能を1つ削る形で作用しました。

Damien Neil の「新しいものを足すより、初回リリースから機能を一時的に取り下げるほうがずっとありそうだ」は、比喩ではありませんでした。
...
タグなしはエラー、タグありもエラーです。Go 1.27の標準ライブラリの中に、v2で `time.Duration` を直列化する手段がありません。自前で `Marshaler` を実装するか、v1を使うことになります。
...
>
> （注意: `ExperimentalSupportFormatTag` はエクスポートされているが、内部パッケージにあるため公開利用はできない。`github.com/go-json-experiment/json` モジュールはGo標準ライブラリと同期が保たれており、公開コードから直接参照できる形でこのオプションを公開する）

構図が一周しました。標準ライブラリが動けないから外部にプロトタイプが作られ、それが標準に取り込まれ、そして標準から外された機能に触るために、また同じ外部モジュールを使うことになっています。

なお、この `format` タグの取り下げも、`time.Duration` がエラーになることも、[Go 1.27のリリースノート](https://go.dev/doc/go1.27)には書かれていません。リリースノートで `encoding/json` に触れているのは、先ほど引用した1節だけです。

## サードパーティの対応

`encoding/json` がv2に載り替えたことは、drop-in を売りにしてきたライブラリ群にとって前提の変更です。反応は分かれました。
...
CIはGo 1.27を既定と `nojsonv2` の両方で回すようになりました。「`encoding/json` と同じ結果を返す」という約束を維持するために、追いかける対象が増えています。

対照表に記録されている差のいくつかを挙げます。float64の範囲を超える数値を読んだとき、Go 1.27はエラーを返したうえで代入先を `+Inf` にするが、sonicはエラーを返して代入先を `0` のまま残す。`map[float64]string` を、sonicは拒否するがGo 1.27は符号化できる。`AppendText` と `MarshalText` の両方がある型で、Go 1.27は `AppendText` を優先する。

[`goccy/go-json`](https://github.com/goccy/go-json) については、GitHub上でv2に関する反応を見つけられませんでした。issueにもREADMEにも言及がなく、ロードマップの記述も以前のままです。これは「反応していない」ことの証明ではありません。本人がSNSやブログで書いている可能性はありますし、探した範囲がGitHubに限られています。

[`json-iterator/go`](https://github.com/json-iterator/go) はアーカイブされました。最終更新は2024年5月で、リポジトリは読み取り専用になっています。星の数がもっとも多かったdrop-in置き換えが止まり、その役割が標準に入ったことになります。このライブラリには「100%互換のdrop-in置き換え」を掲げるREADMEと、[「100%互換のdrop-in置き換えではない」というタイトルの未解決issue](https://github.com/json-iterator/go/issues/229)が、8年間並んで残っていました。

もう一つ、記録しておくべきことがあります。[Discussion #63397](https://github.com/golang/go/discussions/63397) の96件のコメントにも、提案 [#71497](https://github.com/golang/go/issues/71497) の229名のコメント投稿者にも、`goccy/go-json` や `bytedance/sonic` の作者は登場しません。コメント本文にこれらのライブラリ名が挙がることもありませんでした。これもGitHub上で確認できた範囲の話で、Gerritやメーリングリストでのやりとりまでは追えていません。ただ、drop-in置き換えのエコシステムが、自分たちの後継の設計にGitHub上では加わっていなかったのは確かです。

## protojsonとjsontextの現状

`jsontext` を生んだのは、`encoding/json` が使えなかった [`protojson`](https://pkg.go.dev/google.golang.org/protobuf/encoding/protojson) でした。その `protojson` は、いま `jsontext` を使っているのか。
...
>
> （`internal/encoding/json` パッケージ全体を削除し、将来の `encoding/json/jsontext` パッケージで置き換えられる。実のところ `internal/encoding/json` は、最終的に `jsontext` になったものの初期プロトタイプと考えてよい）
...
`proto.Message` をフィールドに含む大きなGoの値を、その部分だけ `protojson` に任せながら一度に直列化する形です。

Go Protobuf のメンテナである Michael Stapelberg は、当時「依存を増やしたくないし、vendoringも面倒なので、標準ライブラリに入るのを待つ」という選択をしました。[2026年7月20日のコメント](https://github.com/golang/protobuf/issues/1673#issuecomment-5020103523)には「json/v2 が今月リリースされる。待つという判断は報われたと思う」とあります。
