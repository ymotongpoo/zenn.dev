...
「どの関数の入口か」「どの関数の出口か」は、アタッチした時点で決まっています。OBIは計装対象の関数ごとに、入口用と出口用のeBPFプログラムを組で持っています。上の抜粋の `probe.End` が出口用で、入口用は同じ構造体のもう一方のフィールドから取り出され、関数の先頭アドレスに結び付けられます。出口用のプログラムが結び付けられるのは、その関数から集めた `ReturnOffsets` だけです。そして7章で見たとおり、カーネルはトラップした瞬間のPCを登録の表と突き合わせて、踏まれたのがどのuprobeかを特定し、それに結び付いたプログラムだけを走らせます。`RET` を踏んだ時点で、それがどの関数の出口なのかはカーネルの側で確定しています。

残るのは、同じ**呼び出し**の入口と出口をどう突き合わせるかです。こちらは実行時の判定で、鍵になるのはgoroutineです。入口のプログラムが記録を残し、出口のプログラムが同じgoroutineの記録を引くことで、1回の呼び出しの開始と終了が対応します。そのgoroutineをどうやって識別するのかが、次の難所2の主題です。
