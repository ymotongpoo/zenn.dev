...
7章で見たとおり、uretprobeは関数の入口に置いたuprobeが発火した時点で、スタックに積まれている戻りアドレスを、カーネルが用意したトランポリンのアドレスに書き換えます。5章で見たとおり、戻りアドレスはスタック上の1スロットに書かれたただの数値で、関数末尾の `RET` 命令はそれを取り出してジャンプするだけです。そのスロットは、入口の時点では `SP` が指しています。スタックの積み降ろしは後入れ先出しに限られ、そこに最後に積まれたものが戻りアドレスだからです。カーネルは `SP` の指す8バイトを読んで退避し、同じ場所へ別の値を書き込むだけで済みます。だからその数値を書き換えれば、`RET` の行き先を変えられます。

つまりuretprobeは「書き込んだ戻りアドレスは、`RET` の瞬間まで同じ場所に残っている」という前提に立っています。

![uretprobe による戻りアドレスの書き換え](/images/20260911-uretprobe-rewrite.png)
*図1: 実線の矢印は制御の流れ、破線は書き換えられた戻りアドレスが導く先を表す。表の中の上下はアドレスの高低で、関数に入った瞬間はSPが戻りアドレスのスロットを指している。uretprobeはそのスロットを、カーネルが対象プロセスのアドレス空間に張ったトランポリン（`[uprobes]` 領域）のアドレスへ書き換える。この方式は「戻りアドレスを置いたスタック上の場所は、その後も動かない」ことを前提にしている。*
...
辿るというのが何をすることなのか、具体的に見ておきます。ランタイムはまず新しい領域を確保して、いま使っている範囲を丸ごとコピーします。そのあと、フレームを1つずつ辿って、コピーした側のポインタを直していきます。出発点は、止まっているgoroutineのプログラムカウンタとスタックポインタです。ランタイムはプログラムカウンタから、4章で見た `.gopclntab`（命令アドレスからGoの関数名を引く表）を引いて、いま実行中の関数を特定します。関数が分かると、そのフレームの大きさと、フレームのどこにポインタがあるかが分かります。ポインタの位置が分かるから、それらを新しいアドレスへ書き換えられます。フレームの大きさが分かるから、呼び出し元へ戻るアドレスがどこに積まれているかも決まります。その戻りアドレスを読んで、また同じ表を引けば、次に処理する呼び出し元の関数が分かります。あとはそちらへ移って、goroutineの起点まで同じことを繰り返します。

つまり1フレームを処理するたびに、「読み取った戻りアドレスから関数を引く」ことが次の一歩の前提になります。関数が引けなければ、そこから先のフレームの大きさが分からず、走査はそこで止まります。値を読めるかどうかではなく、読んだ値から関数に辿り着けるかどうかが問題になります。

![スタックを移すときの1フレームずつの走査](/images/20260911-stack-walk.png)
*図2: 実線の矢印は処理の順序、破線は呼び出し元のフレームへ移って同じ手順を繰り返すことを表す。いま実行中の関数はプログラムカウンタから引き、その関数の情報からフレームの大きさとポインタの位置が決まる。*
...
![戻りアドレスから関数を引けないと、スタックを辿れない](/images/20260911-unwind-failure.png)
*図3: 実線の矢印は処理の流れ、破線はスタックから読み取る値を表す。ランタイムは読み取った戻りアドレスを手がかりに「呼び出し元はどの関数か」を `.gopclntab` から引く。トランポリンはGoのリンカが知らない領域なのでこの表に登録されておらず、そこで辿れなくなる。*
...
ここで、uretprobeとの手間の差がはっきりします。uretprobeなら、知っておくべきアドレスは関数の入口だけでした。出口の捕捉はカーネルが引き受け、入口で退避した戻りアドレスをトランポリンのアドレスに書き換えることで代行します。`RET` がどのアドレスにあるかを、計装する側は知らなくて済みました。OBIの方式はその代行を使わないので、出口のアドレスを自分で全部見つけることになります。シンボルテーブルが教えるのは関数の先頭アドレスとその大きさまでで、`RET` がその範囲のどこにあるかは記録されていません。だから手順1の逆アセンブルが必要になります。
...
`RET` の数だけプローブが増えることの代償も見ておきます。増えるのは、関数が呼ばれるたびのコストではありません。1回の呼び出しで実行される `RET` は1つだけなので、発火する回数はuretprobeのときと変わりません。増えるのは、仕掛けるときと片付けるときのコストです。カーネルへの登録は `RET` 1つにつき1回必要で、この章の後半で見る `instrumenter.go` が、集めたオフセットをひとつずつ回して `exe.Uprobe` を呼び、返ってきたリンクを控えているのがそれです。計装をやめるときは控えたぶんを全部閉じます。計装対象の関数が増えれば、この回数は関数の数と `RET` の数の掛け算になります。解析の側でも、関数ごとに機械語を先頭から1命令ずつ走査することになります。

この方式で取れない出口もあります。パニックによる巻き戻しで飛ばされた関数です。パニックが `recover` されない場合はプログラム全体が終了しますが、呼び出し元の `defer` で `recover` された場合も、そのあいだに飛ばされた内側の関数は `RET` を実行しません。`RET` の経路に戻るのは `recover` を実行した `defer` を登録した関数だけです。`runtime.Goexit` や `os.Exit` でも同じことが起きます。`net/http` のサーバーはハンドラのパニックを `conn.serve` で `recover` するので、ハンドラの内側で計装している関数の出口は、この形で取り逃がされます。もっともその場合はリクエスト自体が失敗しているので、所要時間の記録が残らないことによる実際の影響は限られます。

![全 RET 命令へのuprobe](/images/20260911-ret-uprobes.png)
...
2つ目の経路がいつ選ばれるのかも押さえておきます。`defer` に登録した処理は、通常ならコンパイラが関数の末尾に展開してそのまま実行します。`runtime.deferreturn` を通るのは、その関数自身の `defer` が `recover` でパニックから復帰し、この関数の復帰経路に戻ってくる場合です。ループの中の `defer` のようにコンパイラが展開できない場合も、通常の終了で同じ経路を通ります。したがって `0x49e290` にしかuprobeを置かないと、出口を取り逃す呼び出しが出ます。

ただし、いま見ている `Lookup` の `defer` は `mu.Unlock()` で、`recover` はしません。この関数の中でパニックが起きれば、2つ目の `RET` にも戻らずに巻き戻りが外へ抜けていきます。先に見たとおり、外側で `recover` されても、飛ばされた `Lookup` はどちらの `RET` も通りません。つまり `RET` が2つあることと、パニック時にその2つ目へ来ることは別の話です。どの経路で抜けるかを呼び出しごとに見分けられないからこそ、OBIは全部の `RET` にuprobeを置きます。

ついでに、この出力にはここまでの説明と難所2に関係する要素が2つ写り込んでいます。1行目の `CMPQ SP, 0x10(R14)` は、いま説明したスタック伸長の判定です。`R14` の指す構造体の16バイト目に入っている値とスタックポインタを比べ、足りなければ最終行の `runtime.morestack_noctxt` へ飛びます。その `R14` が何なのかは、難所2で扱います。
...
`exe.Uprobe` の第1引数が空文字列で、`Address` にオフセットを渡している点が、7章で見た「アドレス直指定」の形です。この `offset` がファイル内のオフセットであることも押さえておきます。7章で触れたとおり、`Address` に渡すのは仮想アドレスではありません。OBIは `pkg/internal/goexec/instructions.go` で、逆アセンブルに使った仮想アドレスをELFのセグメント情報からファイル内オフセットへ変換しています。エラーメッセージには `uretprobe` と書かれていますが、実際に呼んでいるのは通常のuprobeです。出口フックという役割の名前だけが残っていて、中身は `RET` の位置に置いた入口フックと同じものになっています。

ここまでで、入口に1つ、出口に `RET` の数だけプローブが並びました。では、入口のプローブと出口のプローブが同じ関数のものであることは、どう分かるのでしょうか。実行時に「いまどの関数の中にいるか」を判定しているわけではありません。判定は2段に分かれています。

「どの関数の入口か」「どの関数の出口か」は、アタッチした時点で決まっています。OBIは計装対象の関数ごとに、入口用と出口用のeBPFプログラムを組で持っています。上の抜粋の `probe.End` が出口用で、入口用は同じ構造体のもう一方のフィールドから取り出され、関数の先頭アドレスに結び付けられます。出口用のプログラムが結び付けられるのは、その関数から集めた `ReturnOffsets` だけです。そして7章で見たとおり、カーネルはトラップした時点のPCを登録の表と照合して、踏まれたのがどのuprobeかを特定し、それに結び付いたプログラムだけを走らせます。`RET` を踏んだ時点で、それがどの関数の出口なのかはカーネルの側で確定しています。

残るのは、同じ**呼び出し**の入口と出口をどう対応づけるかです。こちらは実行時の判定で、鍵になるのはgoroutineです。入口のプログラムが記録を残し、出口のプログラムが同じgoroutineの記録を引くことで、1回の呼び出しの開始と終了が対応します。そのgoroutineをどうやって識別するのかが、次の難所2の主題です。
