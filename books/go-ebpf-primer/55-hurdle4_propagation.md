...
漏れたときに何が起きるかも押さえておきます。トレース情報を持つ祖先までの距離が6段を超えると、`find_parent_goroutine` は0を返します。このとき送信側の処理は計装されないのではなく、`client_trace_parent` が新しいトレースIDを乱数で作ります。つまり下流のリクエストは、上流とつながらない別のトレースとして記録されます。トレースが消える場合より対処が難しく、1本のはずの流れが2本に見えます。
