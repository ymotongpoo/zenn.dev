...
この難所の症状は難所1と対照的です。スタック渡しを前提にした汎用のeBPFツールでGo 1.17以降のバイナリの引数を読むと、何も落ちず、エラーにもならず、でたらめな値が返ります。誤った値が、それらしい顔をして流れてくるだけです。
...
Go 1.17以降は、性能向上のため引数をCPUレジスタで渡すようになりました[^regabi-arch]。これが**ABIInternal**と呼ばれる規約です。amd64では、整数引数は次の順序でレジスタに割り当てられます。
...
この変更でGoの関数呼び出しは速くなりましたが、外から引数を読む側は、どの引数がどのレジスタに入るのかをアーキテクチャごとに知っていなければならなくなりました。汎用のeBPFツールは引数がスタックにあると仮定して作られているので、Goのバイナリに向けるとスタックの決まった位置を読み、そこにある無関係な値を引数として返します。

[^regabi-arch]: Go 1.17でレジスタ渡しに切り替わったのはamd64だけです。arm64を含む他のアーキテクチャは、Go 1.18で切り替わりました。本書の記述はamd64を前提にしています。
