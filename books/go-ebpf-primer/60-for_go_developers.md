...
OBIのサポートマトリクスは、ライブラリレベルの計装について `Go 1.17+` と明記しています。ABIInternal（Go関数が引数を受け渡す内部規約）に切り替わったバージョンが境目です。`offsets.json` に載っている標準ライブラリのエントリも、すべて `oldest` が `1.17.0` です。
