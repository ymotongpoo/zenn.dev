...
- **SP（スタックポインタ）**：スタックの先端、つまり最後に積まれた場所を指すレジスタ。正式名はRSP。関数の先頭の命令に着いた時点では、直前の `CALL` が積んだ戻りアドレスを指している（2章、5章、難所1）。
...
- **ASLR**：スタックやヒープの配置を起動ごとにランダム化するセキュリティ機構（3章）。Goのバイナリでは、これとは別にランタイム自身がヒープの基点を乱択する。
...
- **後入れ先出し（LIFO）**：最後に積んだものを最初に降ろす順序。関数は呼んだ順の逆にしか戻らないので、スタックの積み降ろしはこの順に限られる（5章）。
- **戻りアドレス**：`CALL` がスタックに積む「呼び出しの次の命令のアドレス」。`RET` がそれを取り出してジャンプする。関数の先頭の命令に着いた時点では、`SP` がそのスロットを指している（5章、難所1）。
...
- **ABIInternal**：Goが内部で使う呼び出し規約。名前はGo 1.12からあり、amd64ではGo 1.17から引数をレジスタで渡す（難所2）。
- **ABI0**：アセンブリから呼ぶための安定した呼び出し規約。引数をスタックで渡す。ABIInternalとの行き来にはラッパーが挟まる（難所2）。
...
- **uprobeとuretprobe**：命令アドレスの1バイトをブレークポイント命令に差し替えて置くフックと、関数の入口でスタック上の戻りアドレスを書き換えて復帰を捉える仕組み（7章、難所1）。
...
> 本書の実行結果には、アドレス値のように実行ごとに変わるものが含まれます。確認に使ったGoとOBIとカーネルのバージョンは、1章の冒頭にまとめてあります。
