...
デフォルトの方式は、プラットフォームが変更できる場所によって決まります。本書は、共通のCIテンプレートを持ち、Kubernetesを中心に使う組織では、Goのゼロコード計装にotelcを推奨します。ビルドコマンドの差し替えで展開でき、特権が不要で、計装の更新を通常のデプロイに含められるためです。
...
Goの注入はデフォルトで無効です。Operatorへ `--enable-go-instrumentation=true` フラグを指定し、対象の実行ファイルパスを示すannotationも追加します。
