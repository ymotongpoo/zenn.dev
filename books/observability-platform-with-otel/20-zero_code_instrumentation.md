...
JavaやPythonのように動的なランタイムを持つ言語では、実行時にコードを差し替えられます。静的にコンパイルされたGoの単一バイナリには、実行時にエージェントを読み込む機構がありません。そのため、Goではカーネル側から観測するeBPF方式か、コンパイル時に計装コードを加えるコンパイル時方式を使います。
...
デフォルトの方式は、プラットフォームが変更できる場所によって決まります。本書では、共通のCIテンプレートを持ち、Kubernetesを中心に使う組織では、Goのゼロコード計装にotelcを推奨します。ビルドコマンドの差し替えで展開でき、特権が不要で、計装の更新を通常のデプロイに含められるためです。これはOpenTelemetryが定めた優先順位ではなく、本書での判断です。

バイナリを再ビルドできず、CIも変更できない場合はeBPF方式を選びます。複数の言語をホスト単位でまとめて計装する場合も、言語を問わず動作するOBIが候補になります。
...
*図2　矢印は処理の時間順を表します。開発チームはアノテーションを指定し、OperatorのAdmission WebhookがPod作成時に計装を注入します。*

Goの注入はデフォルトで無効です。Operatorへ `--enable-go-instrumentation=true` フラグを指定し、対象の実行ファイルパスを示すアノテーションも追加します。
...
どの方式でも、送信先には各環境のエージェント Collector（4章）を指定します。ゼロコード計装だけがバックエンドへ直接送信すると、Collectorで行う変換や制御を適用できません。

## 手動計装への段階設計

ゼロコード計装でHTTPの入出力を記録しても、内部のどの処理に時間がかかったのか、どの顧客に関する処理だったのかまでは分かりません。こうした業務固有の情報には手動計装が必要です。ここでプラットフォームチームが用意するのは、手動計装そのものではなく、段階を選べる状態です。まずゼロコード計装で最低限の情報が記録され、必要になったチームが自分の判断で手動計装を足せるようにします。どこまで記録するかを中央が決めるのではなく、開発チームが選べることがセルフサービスの要点です。

ゼロコード計装と手動計装を併用すると、計装が重複する場合があります。たとえば、ゼロコード計装とディストリビューションのotelhttpが同じHTTPリクエストのスパンを作れば、同じ処理を二重に記録します。本書では手動計装を基準とし、SDKによる計装が入ったサービスをゼロコード計装の対象から外します。Operatorではアノテーションを外し、ホスト単位のOBIでは対象プロセスの選択条件から除外します。

SDKディストリビューションは変更できるコードへ組織のデフォルト値を配り、ゼロコード計装はコードを変更できないサービスへ最低限の計装を配ります。どちらを先に導入するかではなく、変更できる位置に応じて使い分けます。
