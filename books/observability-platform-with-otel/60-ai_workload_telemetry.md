...
これらを計装するには、モデル、トークン数、処理段階、品質評価などの記録方法を定義します。50章のレジストリで、属性の名前、型、記録条件を管理します。
...
セマンティック規約のガバナンス（50章）には、gen_ai属性群をSHA固定の依存として取り込み、AI固有の社内属性を `com.example.*` に定義します。機能名やプロンプトのバージョンなどが該当します。規約がDevelopment段階だからこそ、改名への追従をレジストリのdiffと生成で機械化しておきます。

SDKディストリビューション（10章）には、LLM呼び出し計装のヘルパーと、トークン使用量メトリクスのデフォルトの計装を足します。Goに公式計装がない現状では、ディストリビューションがその穴を埋めます。

Collector層（30章）には、AI向けの統制を足します。本文を含むテレメトリーの分離パイプライン、既知パターンのredaction、トークン使用量からのコスト集計、大きなスパンへのサイズ制限は、いずれもgatewayの仕事になります。
