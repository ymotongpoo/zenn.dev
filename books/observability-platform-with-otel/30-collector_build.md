...
*図1　矢印はテレメトリーの流れを表します。二つのノードのエージェントは同じ役割で、全ノードに同じ設定を配ります。組織の方針はゲートウェイへ集約します。破線の振り分け層は、tail samplingを水平スケールさせる場合にだけ追加します。*

## ゲートウェイ層の処理設計

ゲートウェイ層には、組織単位で判断する処理を置きます。

**tail sampling**は、トレースが完結した後に、エラーの有無やレイテンシから保存するかを決めます。たとえば、エラーになったトレースをすべて残し、正常なトレースの1%を残す方針を実装できます。判定には一つのトレースを構成する全スパンが必要なため、ゲートウェイへ配置します。

ゲートウェイを水平スケールさせる場合は、同じtrace IDのスパンを同じゲートウェイインスタンスへ送るload-balancing エクスポーターが前段に必要です。tail samplingを使う二段構成は、実際には振り分け層を含みます。また、tail samplingは保存量を減らしますが、ゲートウェイまでの流量は減らしません。流量はSDK側のhead samplingと分担して制御します。
