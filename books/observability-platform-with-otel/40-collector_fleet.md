...
起動失敗についても、alpha実装を運用上の保証にはできません。実測した0.159.0のSupervisorは、`automatic_config_rollback` を有効にしていても自己復旧できず、起動に失敗した設定を「最後に動作した設定」として永続化しました。復旧は修正済み設定の再配布によります。サーバー側には、エージェントがFAILEDを報告した設定を再送しない制御も必要でした。実験の経緯と数値は9章に記載します。
