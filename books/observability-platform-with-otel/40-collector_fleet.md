...
ゲートウェイ層のtail sampling条件は、設定リポジトリへのPRをマージし、GitOpsで数台のゲートウェイへ反映できます。
...
設定配布の被害は、次の手立てで抑えます。canaryから始める段階展開、Collectorの送受信件数とエクスポーター失敗率の監視、合成テレメトリーの到着確認です。あわせて、展開の停止条件と、修正版を再配布する手順を用意します。自動ロールバックが動作しても、これらの検査と復旧手順を置き換えるものではありません。
...
本番の管理プレーンには、次の機能が必要です。エージェントの認証と識別、テナント分離、対象グループの管理、状態の永続化、変更監査です。さらに、サーバー自身の冗長化とアップグレード、段階展開、競合する更新の処理も必要になります。設定を送信する処理だけで工数を見積もると、これらの運用機能が抜けます。
...
数百ノードのエージェントを管理する構成では、Gitを設定の正となる定義とし、CIで実効設定を検査してから、OpAMPで各Collectorへ配布します。canaryと監視が異常な設定の展開を止め、失敗時には修正版を再配布します。OpAMPは、レビューと検査を終えた設定をフリートへ反映する経路を、Collectorのデプロイから分離します。
