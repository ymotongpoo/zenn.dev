...
自動ロールバックが検出するのは、Collectorが起動できない失敗です。構文上は正しくても、Collectorが起動してしまうために検出されない設定があります。全データをfilterで捨てる設定、誤った宛先、認証に失敗する設定、処理容量を超える設定です。リファレンス実装（80章）で全スパンをdropする設定を配ったところ、ステータスはAPPLIEDでhealthyのまま、テレメトリーだけが停止しました。

起動失敗についても、alpha実装を運用上の保証にはできません。実測した0.159.0のSupervisorは、`automatic_config_rollback` を有効にしていても自己復旧できず、起動に失敗した設定を「最後に動作した設定」として永続化しました。復旧は修正済み設定の再配布によります。サーバー側には、エージェントがFAILEDを報告した設定を再送しない制御も必要でした。実験の経緯と数値は80章に記載します。

設定配布の被害は、次の手立てで抑えます。canaryから始める段階展開、Collectorの送受信件数とexporter失敗率の監視、合成テレメトリーの到着確認です。あわせて、展開の停止条件と、修正版を再配布する手順を用意します。自動ロールバックが動作しても、これらの検査と復旧手順を置き換えるものではありません。
...
数百ノードのagentを管理する構成では、Gitを設定の正となる定義とし、CIで実効設定を検査してから、OpAMPで各Collectorへ配布します。canaryと監視が異常な設定の展開を止め、失敗時には修正版を再配布します。OpAMPは、レビューと検査を終えた設定をフリートへ反映する経路を、Collectorのデプロイから分離します。
