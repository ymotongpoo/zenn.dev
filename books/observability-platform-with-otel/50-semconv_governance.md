...
live-checkはそれ自体がOTLPの受信口になります。gRPCでテレメトリーを受け取り、スパン、メトリクス、ログ、リソースの属性をレジストリと照合して、未登録の属性、型の不一致、非推奨属性の使用などを報告します。判定にはRegoポリシーを追加でき、違反があれば終了コードが非ゼロになるため、CIに組み込めます。
