...
- 必要に応じて、ゲートウェイ（4章）の属性変換設定も同じレジストリから生成する。非推奨になった属性を新しい名前へ書き換えるtransform設定は、改名の情報から機械的に導出できる。このとき4章で述べたとおり、変換は入力のschema URLと出力のschema URLを一組にして生成し、属性だけを書き換えて宣言が古いまま、という不整合を作らない
