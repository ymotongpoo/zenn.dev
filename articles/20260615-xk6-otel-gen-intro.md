...
最近でこそAIコーディングエージェントでマイクロサービスを実装して、Kubernetesクラスターをセットアップして、デプロイするという手順を昔よりもずっと短時間でできるようになりました。
とはいえ、デモをするのにマイクロサービスを手元で動かすのもちょっと面倒ですし、まして公開デモをする場合にはクラウドサービスでKubernetesを動かし続けないといけないので、お金もかかります。
...
これは名前からわかるようにk6拡張です。
リポジトリを見れば一目瞭然で、AIコーディングエージェントに全部実装してもらいました。
...
k6スクリプトからジャーニーを実行するたびに、エンジンがエッジを再帰的にたどり、レイテンシー分布(対数正規分布など)とエラー率に従ってスパンを合成します。
スパンに対応するメトリクス（レイテンシーヒストグラムやリクエストカウント）とログも同時に生成され、すべて同じトレースコンテキストで関連付けられます。
...
古典的な3層構成(frontend → backend → database)をモデル化してみます。
次のYAMLを `topology.yaml` として保存してください。
...
サービスのトポロジーは定義できたので、次はシナリオです。
ジャーニーをk6スクリプトとして記述します。
例として次の内容を `script.js` として保存します。
...
構成には1つ注意点があります。
`otelgen.load()` は**`default`関数の中で呼びます**。
k6では `setup()` の戻り値がJSONシリアライズされて各VUに渡されるため、`load()` が返すハンドルを `setup()` から返すとメソッドが失われてしまいます。
`load()` はファイルの読み込みと検証をテスト全体で1回だけ行い、2回目以降はキャッシュ済みハンドルを返すだけなので、毎イテレーションで呼んでもオーバーヘッドはありません。
...
エラースパンには `http.response.status_code=500` と `error.type` が現れます。
また対応するログには `exception.type` / `exception.message` が付与されます。
アラートルールやエラー率ダッシュボードの動作確認が、YAMLを数行書き換えるだけでできるようになりました。

Grafana Tempoでもこのようにトレースで500系のエラーが起きたことが確認できています。
...
思い付きで作り始めたばかりですし、試しながらいろんなバグを発見していますが、盆栽ツールとしては実用になりそうで今のところ気に入っています。
またツールの当初の目的以外に、結構実用的な使い方もできそうな気がしているので、育てながらいろいろなユースケースを試してみようと思います。

興味を持っていただいた方向けに少しだけOSSとしての性格を共有します。
Apache License 2.0で公開はしているものの、仕事でもAI slopに疲れ気味なので、当面pull requestは受け付けません。
そのかわりユースケースの提案や明らかなバグ報告などは歓迎します。
片手間で開発するツールで全部AIに投げて実装するため、詳細なレポートが重要となるためテンプレートを固定しています。
