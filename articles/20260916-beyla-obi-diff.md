...
BeylaとOBIを同じnginxに対して動かして出力を比べたところ、計装エンジンとHTTPサーバーメトリクスは一致しました。差は独自メトリクスの接頭辞、Grafana製品へ繋ぐための設定経路、プロセスメトリクスやsurveyモードといった追加機能に限られます。

デフォルト値には同じ入力で結果が変わる箇所があり、ルート推定のモードがその例です。Beylaはカーディナリティに上限を設けてまとめる側に寄っています。

いま両者の距離がもっとも開いているのは設定ファイルで、OBIだけが宣言的設定に沿ったConfig v2を読めます。Beylaが追いつくにはOBI側が公開ローダーを出す必要があります。
