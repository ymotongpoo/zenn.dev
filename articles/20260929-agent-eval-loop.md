...
良いシナリオの条件も書かれています。ベンチマークのために作られた不自然な問題ではなく、現実のユーザーの仕事に似ていること。その領域に詳しい2人が合否で一致する程度に具体的であること。言い回しが引っかけになっているからでも指示が膨らんでいるからでもなく、正しい理由で難しいこと。そして失敗したときに何かを教えてくれる程度に、扱う範囲が絞られていること。
...
可能なかぎり、ユーザーが実際に体験する結果を評価しようとしている点も見ておきたいところです。保存された Grafana の状態、クエリの構造、パネルのスキーマ、ナビゲーションの目的地、環境から取れる事実。正確な言い回しは、最適化する対象としてはたいてい最も面白くない部分だと書かれています。グレーダーを、判定の基準となる状態や実際に取得した証拠に結び付けられるほど、判定は信頼できるものになり、合否の差もはっきりします。
...
品質とコストとレイテンシは一緒に追います。効率が改善したときは、タスクの完遂に何を支払ったか確かめてから評価すること。
...
オブザーバビリティに馴染んだ読者には、この姿勢はそのまま通じるはずです。計装して、検査して、改善する。違うのは、観測している対象がインフラではなくエージェントの振る舞いだという点だけです。
...
175シナリオの実例が示したのは、合計スコアが判断の材料にならないことでした。同じ改修が discover と safety では改善し、observe と learn と dashboard では退行していた。区分ごとに割った表があって初めて、どの改善を残しどの退行を切り分けるかという判断ができます。

最後に、この仕組み自体が劣化します。シナリオは古び、グレーダーはずれ、環境の前提は成り立たなくなる。評価の基盤をプロダクトと同じように保守の対象として扱わないと、周回は静かに信頼性を失います。
