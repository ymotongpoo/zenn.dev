#!/usr/bin/env python3
"""不可視の経由点ノードで折れ曲がる経路の、角の隙間をふさぐ。

`layout=neato` の絶対座標レイアウトでは、経路を折り曲げるのに
`shape=point, style=invis` の経由点ノードを挟み、`a -> jn; jn -> b;` と
区間ごとに別々の辺を引く。このとき各辺は経由点の手前（約1pt）で切れるため、
角に隙間が残り、`round_edges.py` でも丸まらない（あちらは1本の辺の内部の
折れ角しか扱えない）。この後処理は
...
対象は「軸並行の一直線として描かれている辺」だけで、それ以外の辺
（`layout=dot` + `splines=ortho` が出力する、1本でL字に折れる辺など）は
いっさい書き換えずに素通しする。このガードがないと、折れている辺を
両端だけの直線に潰してしまい、`round_edges.py` が丸めた角が消える。

    使い方: dot -Tsvg foo.dot | round.py | round_edges.py | elbow.py > foo.svg
"""
...
STRAIGHT_EPS = 0.5   # この幅に収まっていれば一直線とみなす
...
def straight_endpoints(d):
    """パスが軸並行の一直線なら両端点を返す。折れていれば None を返す。

    頂点の個数では判定しない。`round_edges.py` を通した後のパスは、
    直線上に制御点が載った `M... L... Q... L...` の形になっていることが
    あるため、すべての点が同じ水平線（または垂直線）上にあるかどうかで
    判定する。
    """
    ...
    xs = [p[0] for p in pts]
    ys = [p[1] for p in pts]
    flat_x = max(xs) - min(xs) <= STRAIGHT_EPS
    flat_y = max(ys) - min(ys) <= STRAIGHT_EPS
...
class Seg:
    def __init__(self, gid, attrs_pre, attrs_post, p0, p1):
        self.gid = gid
        ...
        self.p1 = p1
        self.dead = False
        self.snapped = False   # 交点にスナップされた（＝他の辺と接している）か
...
def main():
    ...
    # エッジグループから、軸並行の一直線として描かれているパスだけを集める。
    # 折れているパスはここで除外し、以降いっさい手を触れない。
    segs = []
    ...
    for gi, g in enumerate(groups):
        ...
        if not m:
            continue
        pair = straight_endpoints(m.group(2))
        if pair is None:
            continue
        segs.append(Seg(gi, m.group(1), m.group(3), *pair))
    ...
    for gi, g in enumerate(groups):
        ...
        if s is not None:
            ...
            if id(s) in replacements:
                ...
                # 交点に接しているが鎖にはならなかった区間（T字の3本目など）。
                # 座標をスナップしたうえで、丸い線端で notch を埋める。
            ...
            # どの交点にも接していない独立した辺は素通しする。ここで
            # 書き換えると、経由点を使っていない図にまで線端の丸めが及ぶ。
        out.append(text)
        last = g.end()
    out.append(svg[last:])
